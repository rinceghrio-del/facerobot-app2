...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.CAMERA)
            ...
            statusText.text = "Naghahanap ng tao... [v6] (hinihintay permission ng camera...)"
    ...
    private fun showEyesUi() {
        ...
        statusText.text = if (yoloDetector.isReady) {
            "Naghahanap ng tao... [v6, frames:$frameCount]"
        } else {
            "Naghahanap ng tao... [v6] (kulang: assets/yolo_person.tflite)"
        }
    }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == 100) {
            if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                statusText.text = "Naghahanap ng tao... [v6] (TINANGGIHAN ang camera permission)"
            }
        }
    }

    // Bilang ng frames na natanggap mula sa camera (kahit throttled/skipped) - para
    // malaman natin nang tiyak kung tumatakbo ba talaga ang analyzer o hindi.
    private var frameCount = 0

    private fun processFrame(imageProxy: ImageProxy) {
        frameCount++
        if (appState == AppState.EYES) {
            // Palaging ipakita ang bilang ng frames, kahit throttled pa ang aktwal na
            // YOLO check - kung hindi kumikilos ang numerong ito, ibig sabihin hindi
            // talaga tumatakbo ang camera analyzer (bind/permission na ang dahilan,
            // hindi na YOLO/model issue).
            val fc = frameCount
            runOnUi {
                if (appState == AppState.EYES) {
                    statusText.text = "Naghahanap ng tao... [frames:$fc]"
                }
            }
        }
        when (appState) {
        ...
        try {
            ...
            runOnUi {
                ...
                statusText.text = when {
                    err != null -> "Naghahanap ng tao... [frames:$frameCount] (YOLO error: $err)"
                    else -> "Naghahanap ng tao... [frames:$frameCount] (confidence: %.2f)".format(yoloDetector.lastMaxPersonScore)
                }
            }
